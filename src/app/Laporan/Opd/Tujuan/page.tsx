...
import TableLaporanTujuanOpd from './table'
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow mt-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Laporan Realisasi Tujuan OPD</h2>
        <Select
          ...
          onChange={(opt) => {
            ...
            if (opt) {
              localStorage.setItem('selectedLaporanOpdTujuan', JSON.stringify(opt))
            } else {
              localStorage.removeItem('selectedLaporanOpdTujuan')
            }
          }}
      ...
      {!isLoaded ? null : !selectedLaporanOption ? (
        <div className="p-5 bg-red-100 border border-red-400 rounded text-red-700 my-5">
          Pilih laporan realisasi agar data laporan tujuan opd muncul.
        </div>
      ) : (
        <TableLaporanTujuanOpd laporanType={selectedLaporanOption?.value} />
      )}
    </div>
  )
}
