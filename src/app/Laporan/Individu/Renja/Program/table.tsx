...
export default function TableLaporanRenjaProgram({ laporanType }: { laporanType?: string }) {
    ...
    if (laporanType && activatedTahun && kodeOpd) {
        ...
        if (isAdmin) {
            if (activatedLevelRole && activatedNamaPegawai) {
                ...
                url = `/api/v1/realisasi/renja_individu/program/laporan/kodeOpd/${kodeOpd}/tahun/${tahun}/jenisLaporan/${jenisLaporan}/levelRole/${activatedLevelRole}/nip/${safeNip}`;
            }
        } else if (nip) {
            const safeNip = nip.replace(/-$/, "");
            url = `/api/v1/realisasi/renja_individu/program/laporan/nip/${safeNip}/kodeOpd/${kodeOpd}/tahun/${tahun}/jenisLaporan/${jenisLaporan}`;
        }
