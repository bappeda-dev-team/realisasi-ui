...
export default function TableLaporanRenjaSubKegiatan({ laporanType }: { laporanType?: string }) {
    ...
    if (laporanType && activatedTahun && kodeOpd) {
        ...
        if (isAdmin) {
            if (activatedLevelRole && activatedNamaPegawai) {
                ...
                url = `/api/v1/realisasi/renja_individu/subkegiatan/laporan/kodeOpd/${kodeOpd}/tahun/${tahun}/jenisLaporan/${jenisLaporan}/levelRole/${activatedLevelRole}/nip/${safeNip}`;
            }
        } else if (nip) {
            const safeNip = nip.replace(/-$/, "");
            url = `/api/v1/realisasi/renja_individu/subkegiatan/laporan/nip/${safeNip}/kodeOpd/${kodeOpd}/tahun/${tahun}/jenisLaporan/${jenisLaporan}`;
        }
