...
export default function TableLaporanTujuan({ laporanType }: { laporanType?: string }) {
    ...
    if (laporanType && activatedTahun) {
        ...
        fetchUrl = `/api/v1/realisasi/tujuans/laporan/tahun/${tahun}/jenisLaporan/${jenisLaporan}${queryString ? `?${queryString}` : ''}`;
    }
            ...
            <table className="w-full">
                ...
                <tbody>
                    {loading ? (
                        ...
                        <tr>
                            <td colSpan={isTahunan ? 16 : isTriwulan ? 8 : 4} className="text-center py-4 text-gray-500">Pilih laporan realisasi agar data laporan tujuan pemda muncul.</td>
                        </tr>
                                ...
                                {isTahunan ? (
                                    <>
                                        ...
                                        {[...Array(12)].map((_, i) => (
                                            <td key={i} className="border border-emerald-400 px-2 py-4 text-center">
                                                {item.listData?.[i + 1] !== undefined ? item.listData[i + 1] : '-'}
                                            </td>
                                        ))}
                                    ...
                                    <>
                                        ...
                                        <td className="border border-emerald-400 px-6 py-4 text-center">
                                            {item.listData && Object.values(item.listData).length > 0 ? String(Object.values(item.listData)[0]) : '-'}
                                        </td>
                                    </>
                                )}
