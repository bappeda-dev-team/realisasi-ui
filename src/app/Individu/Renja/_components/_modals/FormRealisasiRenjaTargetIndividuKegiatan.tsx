...
import { FormProps, RenjaTarget, RenjaKegiatanIndividuResponse } from "@/types";
import { useFilterContext } from "@/context/FilterContext";
import { useUserContext } from "@/context/UserContext";
import { useSubmitData } from "@/hooks/useSubmitData";
import { getMonthKey, getMonthName } from "@/lib/months";

type FormRealisasiRenjaTargetIndividuKegiatanProps = FormProps<RenjaTarget[], RenjaTarget[]>;

interface KegiatanRealisasiRequest {
  kodeOpd: string;
  tahun: string;
  bulan: string;
  nip: string;
...
const FormRealisasiRenjaTargetIndividuKegiatan: React.FC<FormRealisasiRenjaTargetIndividuKegiatanProps> = ({ requestValues, onClose, onSuccess }) => {
  ...
  const submitUrl = '/api/v1/realisasi/renja_individu/kegiatan';

  const { submit, loading, error } = useSubmitData<RenjaKegiatanIndividuResponse>({ url: submitUrl });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: KegiatanRealisasiRequest = {
...
export default FormRealisasiRenjaTargetIndividuKegiatan;
