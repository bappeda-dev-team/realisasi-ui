...
const SasaranOpdTable = () => {
    ...
    const [pdfFileName, setPdfFileName] = useState<string>("renja-individu-sasaran-opd.pdf");
    ...
    const createPdfDocument = () => {
        ...
        doc.setFontSize(14);
        doc.text(`Renja Individu - Sasaran OPD${opdTitle}`, 40, 40);
        doc.setFontSize(10);
        ...
        const fileName = `renja-individu-sasaran-opd-${safeYearLabel}-${safeMonthLabel}.pdf`;
        return { doc, fileName };
    };
        ...
        <>
            <div className="m-2 rounded-t-xl">
                <div className="flex justify-between items-center px-4 py-3">
                    <h3 className="font-semibold text-gray-800 text-lg">Sasaran OPD</h3>
                </div>
                {renderSyncButton()}
                {loading ? (
                    ...
                    <div className="overflow-auto">
                        <table id="print-area-rekin" className="w-full">
                            ...
                            <tbody>
                                {rows.map((item, index) => {
                                    const targets = item.targets && item.targets.length > 0 ? item.targets : [null];
                                    return targets.map((target, targetIndex) => (
                                        <tr key={`${item.id}-${target?.targetId || targetIndex}`}>
                                            {targetIndex === 0 && (
                                                <>
                                                    <td rowSpan={targets.length} className="border-x border-b border-emerald-500 py-4 px-3 text-center">
                                                        {index + 1}
                                                    </td>
...
export default SasaranOpdTable;
