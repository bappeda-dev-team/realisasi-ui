...
const KegiatanTable = () => {
    ...
    if (effectiveKodeOpd && activatedTahun && bulanKey) {
        if (isOpdScopedView) {
            ...
            apiUrl = `/api/v1/realisasi/renja_individu/kegiatan/kodeOpd/${encodeURIComponent(effectiveKodeOpd)}/nip/${encodeURIComponent(effectiveNip)}/tahun/${encodeURIComponent(activatedTahun)}/bulan/${encodeURIComponent(bulanKey)}`;
        }
    }
