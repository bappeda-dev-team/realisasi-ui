...
interface FormFaktorPenunjangRenaksiOpdProps {
  kodeOpd: string;
  tahun: string;
  bulan: string;
  rekinId: string;
...
const FormFaktorPenunjangRenaksiOpd: React.FC<FormFaktorPenunjangRenaksiOpdProps> = ({
  kodeOpd,
  tahun,
  bulan,
  rekinId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/v1/realisasi/renaksi_opd/faktor-penunjang', {
        method: 'POST',
        headers: {
...
export default FormFaktorPenunjangRenaksiOpd;
