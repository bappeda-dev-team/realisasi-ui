...
interface FormFaktorPenghambatRenaksiOpdProps {
  kodeOpd: string;
  tahun: string;
  bulan: string;
  rekinId: string;
...
const FormFaktorPenghambatRenaksiOpd: React.FC<FormFaktorPenghambatRenaksiOpdProps> = ({
  kodeOpd,
  tahun,
  bulan,
  rekinId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/v1/realisasi/renaksi_opd/faktor-penghambat', {
        method: 'POST',
        headers: {
...
export default FormFaktorPenghambatRenaksiOpd;
