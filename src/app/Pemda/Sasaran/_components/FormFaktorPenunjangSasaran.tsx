...
interface FormFaktorPenunjangSasaranProps {
  sasaranId: string;
  indikatorId: string;
  targetId: string;
  tahun: string;
...
const FormFaktorPenunjangSasaran: React.FC<FormFaktorPenunjangSasaranProps> = ({
  sasaranId,
  indikatorId,
  targetId,
  tahun,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${url}/api/v1/realisasi/sasarans/faktor-penunjang`, {
        method: 'POST',
        headers: {
...
export default FormFaktorPenunjangSasaran;
