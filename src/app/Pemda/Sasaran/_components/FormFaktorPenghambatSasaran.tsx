...
interface FormFaktorPenghambatSasaranProps {
  sasaranId: string;
  indikatorId: string;
  targetId: string;
  tahun: string;
...
const FormFaktorPenghambatSasaran: React.FC<FormFaktorPenghambatSasaranProps> = ({
  sasaranId,
  indikatorId,
  targetId,
  tahun,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${url}/api/v1/realisasi/sasarans/faktor-penghambat`, {
        method: 'POST',
        headers: {
...
export default FormFaktorPenghambatSasaran;
